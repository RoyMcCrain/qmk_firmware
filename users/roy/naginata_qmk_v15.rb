...
r_kana    = %w[a i u e o ka ki ku ke ko sa si su se so ta ti tu te to na ni nu ne no ha hi hu he ho ma mi mu me mo ya
...
def teigi(a, b, c, prefix = '', suffix = '')
  ...
  format('  {%-32s', ".key = #{prefix}#{_a}#{suffix}") + format('%-20s', ", .kana = \"#{b}\"") + "}, // #{c}"
end

puts '  // 清音'
kana.each_with_index do |k, i|
  j = tanda.index(k)
  puts teigi(eiji[j], r_kana[i], k) if j && j >= 0
  j = shifted.index(k)
  puts teigi(eiji[j], r_kana[i], k, 'B_SHFT|') if j && j >= 0
end
